...
int token_read(Token *token, bool *can_be_unary) // никаких проверок правильности ввода не делала. Нужны?
{
    char c;
    c = fgetc(stdin);
...
void token_print(Token *token)
{
    ...
    if (token_is_double_value(token)) {
        printf("%0.4lf", token->data.double_value);
    }
